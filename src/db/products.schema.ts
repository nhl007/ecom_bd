...
export const orders = pgTable("order", {
  serial: serial("sl"),
  id: uuid("invoice_id").defaultRandom().primaryKey(),
  ...
  delivery: integer("delivery_charge").$default(() => 60),
});

export const categories = pgTable("category", {
  serial: serial("SI"),
  category: varchar("category", { length: 60 }).notNull().unique().primaryKey(),
  status: boolean("category_status")
    .$default(() => true)
    .notNull(),
});
...
export const couriers = pgTable("courier", {
  ...
  city: boolean("city_available")
    .$default(() => false)
    .notNull(),
  zone: boolean("zone_available")
    .$default(() => false)
    .notNull(),
  status: boolean("courier_status")
    .$default(() => true)
    .notNull(),
});
